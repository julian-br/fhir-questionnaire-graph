...
export default function SearchForItemsComboxBox({
  ...
  return (
    <SearchForItemComboBoxDialog onClose={onClose}>
        ...
        <Combobox.Options className="box-border h-fit overflow-y-auto rounded-b-xl bg-slate-100">
          {values.map((value) => (
            <Combobox.Option
              ...
              value={value}
              className={({ active }) =>
                `w-full border-b  py-4 px-4 text-left font-semibold  ${
                  active ? "text-primary-600" : "bg-white text-slate-600"
                }`
              }
            >
              {value}
    ...
    </SearchForItemComboBoxDialog>
  );
}

function SearchForItemComboBoxDialog({
  onClose,
  children,
}: {
  onClose: () => void;
